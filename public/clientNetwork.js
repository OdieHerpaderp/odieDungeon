...
class ClientNetwork {
  ...
  clearWebRTCTimers() {
    if (this.webrtcReconnectTimer) {
      clearTimeout(this.webrtcReconnectTimer);
      this.webrtcReconnectTimer = null;
    }
    if (this.webrtcIceRestartTimer) {
      clearTimeout(this.webrtcIceRestartTimer);
      this.webrtcIceRestartTimer = null;
    }
  }
  ...
  closeWebRTCConnection() {
    ...
    try {
      ...
      console.error("[WebRTC Client] Error closing data channel:", e);
    }
    ...
    try {
      ...
      console.error("[WebRTC Client] Error closing peer connection:", e);
    }
  ...
  initWebRTCHandlers() {
    this.socket.on("webrtc-answer", async (d) => {
      ...
      try {
        await this.webrtcPeer.setRemoteDescription(d.answer);
      } catch (e) {
    ...
    this.socket.on("webrtc-signal", async (d) => {
      if (!this.webrtcPeer || this.webrtcPeer.connectionState === "closed") return;
      try {
        d.candidate && (await this.webrtcPeer.addIceCandidate(d.candidate));
      } catch (e) {
  ...
  initDeltaHandlers() {
    const regHandler = (event, handler, track = false) =>
      this.socket.on(event, (d) => {
  ...
  initUtilityHandlers() {
    ...
    this.socket.on("ping", (ts) => {
      this.trackPacket(this.tcpStats, false);
      this.socket.emit("pong", ts);
  ...
  handleGameDelta(data) {
    ...
    if (data.gear) {
      ...
      if (data.playerUpdates) {
        this._applyEntityUpdates(data.playerUpdates, {
          currentArr: this.currentState.players,
          lastMap: this.lastKnownState.players,
          isPlayer: true,
          applyFn: (cur, last, u) => {
    ...
    if (data.enemyUpdates) {
      this._applyEntityUpdates(data.enemyUpdates, {
        ...
        lastMap: this.lastKnownState.enemies,
        isPlayer: false,
        inTown,
  ...
  calculatePacketsPerSecond(type) {
    const now = Date.now(),
      windowSecs = this.maxTrackingWindow / 1000;
  ...
  getNetworkStatistics() {
    return {
      tcp: {
        ping: this.calculateAveragePing(this.tcpStats),
        packetsPerSecond: this.calculatePacketsPerSecond("tcp"),
        totalSent: this.tcpStats.packetsSent,
        totalReceived: this.tcpStats.packetsReceived,
        bytesSent: this.tcpStats.bytesSent,
        bytesReceived: this.tcpStats.bytesReceived,
        throughputKBps: this.calculateThroughputKBps(this.tcpStats),
      },
      udp: {
        ping: this.calculateAveragePing(this.udpStats),
        packetsPerSecond: this.calculatePacketsPerSecond("udp"),
        totalSent: this.udpStats.packetsSent,
        totalReceived: this.udpStats.packetsReceived,
        bytesSent: this.udpStats.bytesSent,
        bytesReceived: this.udpStats.bytesReceived,
        throughputKBps: this.calculateThroughputKBps(this.udpStats),
        connected: this.udpStats.connected,
      },
    };
  }
  ...
  embarkDungeon(dungeonKey) {
    this._sendAction("embarkDungeon", { dungeon: dungeonKey });
  }
  toggleAutoEmbark(enabled) {
    this._sendAction("toggleAutoEmbark", { enabled });
  }
  escapeDungeon() {
    this._sendAction("escapeDungeon", {});
  }
  ...
  donate() {
    this._sendAction("donate");
  }
  equipItem(slot, itemId) {
    this._sendAction("equipItem", { slot, itemId });
  }
  sellItem(itemId) {
    this._sendAction("sellItem", { itemId });
  }
  changeDungeon(dungeon) {
    this._sendAction("changeDungeon", { dungeon });
  }
  buyGear(type) {
    this.trackPacket(this.tcpStats, true);
  ...
  performCombatAction(actionData) {
    this.sendPreferWebRTC("combatAction", actionData);
  }
  updatePlayerPosition(posData) {
    this.sendPreferWebRTC("playerMove", posData);
  }
  ...
  updatePerformanceStatus() {
    ...
    [
      ["deltaStatus", stats.clientPrediction],
      ["batchStatus", stats.batchEvents],
      ["predStatus", stats.clientPrediction],
    ].forEach(([id, active]) => {
