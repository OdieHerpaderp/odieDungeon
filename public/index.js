...
function refreshDungeonListIfChanged(state) {
  if (!state) return;
  const sig = dungeonListSig(state);
...
function getPrerequisiteDungeon(dungeonKey) {
  ...
  const sortedKeys = keysInOrder.includes("field") ? ["field", ...keysInOrder.filter((k) => k !== "field")] : keysInOrder;
...
function updateEmbarkButtonLabel() {
  ...
  embarkBtn.textContent = currentState?.floor === 0 && !currentState?.combatActive ? `🚀 Embark: ${emoji} ${currentDungeon}` : "🚀 Embark";
}
...
function getOriginalStyle(frameAppearance) {
  ...
  Object.assign(frameAppearance, {
    titleBarHeight: "14px",
    titleBarCaptionFontSize: "13px",
    titleBarCaptionFontWeight: "600",
    titleBarCaptionLeftMargin: "2px",
    titleBarCaptionColorDefault: "gray",
    titleBarCaptionColorFocused: "white",
    titleBarCaptionTextShadow: null,
    titleBarColorDefault: "#161616",
    titleBarColorFocused: "#161616",
    titleBarBorderBottomDefault: null,
    titleBarBorderBottomFocused: null,
    frameBorderRadius: "4px",
    frameBorderWidthDefault: "3px",
    frameBorderWidthFocused: "3px",
    frameBorderColorDefault: "#161616",
    frameBorderColorFocused: "#161616",
    frameBoxShadow: null,
    resizeAreaVisible: false,
    resizeAreaWidth: 5,
    resizeAreaHeight: 5,
  ...
  frameAppearance.onInitialize = function () {
    ...
    Object.assign(closeButtonApr, {
      ...
      backgroundColorHovered: "rgba(33, 33, 33, 0.2)",
      backgroundColorPressed: "rgba(33, 33, 33, 0.2)",
      backgroundBoxShadow: null,
      caption: "\u2716",
      captionColorDefault: "gray",
      captionColorFocused: "white",
      captionColorHovered: "white",
      captionColorPressed: "white",
      captionShiftYpx: 0,
      captionFontRatio: 0.6,
    });
...
export function selectGearTab(prefix, label) {
  ...
  if (bodies) bodies.querySelectorAll(".gear-tab-body").forEach((b) => b.classList.toggle("active", b.dataset.label === active));
};
  ...
  return `
    <div style="padding: 10px; color: white; display: flex; gap: 10px;">
      ...
      <div style="flex: 1;">
        ${optionSection(
          "Perf. Mode",
          `<select id="perfModeSelect" onchange="changePerformanceMode(this.value)" class="select-dark">
        ...
        ${optionSection(
          "Batching Size",
          `<select id="batchSizeSelect" onchange="changeBatchSize(this.value)" class="select-dark">
...
const frameConfigs = [
  ...
  { name: "Shop", title: "🛒 Shop (town only)", left: 935, top: 285, width: 300, height: 260, minWidth: 180, minHeight: 160, html: generateShopHtml() },
  {
    ...
    minWidth: 160,
    minHeight: 80,
    html: '<div id="eventLog" style="width:100%; height:100%; color:white; overflow-y:scroll; font-size:12px; z-index:1000;"></div>',
  },
  {
    name: "FloorControls",
  ...
  },
  {
    ...
    minWidth: 200,
    minHeight: 150,
    html: generatePlayerFrameHtml(),
  },
  {
    name: "Options",
    title: "⚙️ Options & Performance",
    left: 750,
    top: 420,
    width: 310,
    height: 280,
  ...
  },
  {
    ...
    minWidth: 200,
    minHeight: 150,
    html: '<div id="partyMembers" style="width:100%; height:100%; color:white; overflow-y:auto;"></div>',
  },
  {
    name: "Enemies",
    title: "👹 Enemies",
    ...
    minWidth: 150,
    minHeight: 150,
    html: '<div id="enemies" style="width:100%; height:100%; color:white; overflow-y:auto;"></div>',
  },
  {
    name: "CombatLog",
    title: "⚔️ Combat Log",
...
const {
  Skills: skillsFrame,
  AbilitySlots: abilitySlotsFrame,
  Equipment: equipmentFrame,
  Shop: shopFrame,
  Win1: eventFrame,
  FloorControls: floorControlFrame,
  CurrentPlayer: playerFrame,
  Options: optionsFrame,
  PartyMembers: partyMembersFrame,
  Enemies: enemiesFrame,
...
const createWindowManagerFrame = () => {
  ...
  const sync = () =>
    frameConfigs.forEach((c) => {
      const b = document.getElementById("wm-toggle-" + c.name);
      if (b) b.textContent = isVis(frames[c.name]) ? "Hide" : "Show";
    });
  window.wmToggle = (n) => {
    isVis(frames[n]) ? frames[n].hide() : frames[n].show();
    sync();
  };
  window.wmCenter = (n) => frames[n].setPosition(innerWidth / 2, innerHeight / 2, "CENTER_CENTER");
  window.wmShowAll = () => {
    Object.values(frames).forEach((f) => f.show());
    sync();
  };
  window.wmHideAll = () => {
    Object.values(frames).forEach((f) => f.hide());
    sync();
  };
...
function buildClientCallbacks() {
  return {
    ...
    onJoinedParty: (data) => {
      ...
      playerIdElements.clear();
      enemyElements.clear();
      _entityCache.clear();
      // reset lastHp so stale per-frame HP snapshots don't survive a party join
    ...
    onLeaveParty: () => {
      ...
      playerIdElements.clear();
      enemyElements.clear();
      // reset lastHp so stale per-frame HP snapshots don't survive a party leave/join
      for (const k in lastHp) delete lastHp[k];
      document.getElementById("partyMembers").innerHTML = "";
      document.getElementById("enemies").innerHTML = "";
    ...
    onAttack: (data) => {
      ...
      else if (attackerId && (attackerId === ownName || attackerId === ownId)) element = ownPlayerElement;
...
function getCalculatedItem(item, slotKey) {
  ...
  if (!perSlot) {
    perSlot = new Map();
    _itemCalcCache.set(item, perSlot);
  }
...
export function renderEquipmentPanel(player, force = false) {
  ...
  const snapshot = equipmentSig(player);
  ...
  container.innerHTML = EQUIPMENT_CATEGORIES.map((category) => {
    ...
    const groups = groupItems(
      inventory.filter((i) => i && i.id != null && category.slots.includes(i.slot)),
  ...
  if (equipTabs)
    equipTabs.querySelectorAll(".gear-tab").forEach((tab, i) => {
      const label = EQUIPMENT_CATEGORIES[i].label;
      tab.classList.toggle("new-items", label !== _activeEquipCat && _newEquipCats.has(label));
    });
}

// Shop stock change signature - avoids JSON.stringify allocation
function shopStockSig(shopStock) {
  if (!shopStock || !shopStock.length) return "empty";
  return shopStock
    .filter((item) => item && typeof item === "object" && item.id != null)
    .map((item) => `${item.baseItem || item.id}|${item.level ?? 1}|${item.rarity ?? 1}|${item.slot || ""}`)
    .sort()
    .join(",");
}
...
export function renderShopStock(shopStock, force = false) {
  ...
  const snapshot = shopStockSig(shopStock);
  ...
  const groups = groupItems(indexed, (it) => `${it.baseItem || it.id}|${it.level ?? 1}|${it.rarity ?? 1}`);
  ...
  if (shopTabs)
    shopTabs.querySelectorAll(".gear-tab").forEach((tab, i) => {
      const label = EQUIPMENT_CATEGORIES[i].label;
...
export function togglePlayerCard(btn) {
  ...
  const p = el._lastPlayer;
  if (p) el.querySelector(".player-equipped").innerHTML = renderEquippedHtml(p);
};
...
function updatePlayerElement(el, player, canAllocate) {
  ...
  const ui =
    ...
    (el._ui = {
      ...
      mpFill: el.querySelector(".mp-fill"),
      apFill: el.querySelector(".ap-fill"),
      xpText: el.querySelector(".xp-text"),
      pointsText: el.querySelector(".points-text"),
      hpText: el.querySelector(".hp-text"),
      mpText: el.querySelector(".mp-text"),
      apText: el.querySelector(".ap-text"),
      levelDisplay: el.querySelector(".level-display"),
      goldText: el.querySelector(".gold-text"),
      strVal: el.querySelector(".str-val"),
      dexVal: el.querySelector(".dex-val"),
      agiVal: el.querySelector(".agi-val"),
      vitVal: el.querySelector(".vit-val"),
      intVal: el.querySelector(".int-val"),
      cncVal: el.querySelector(".cnc-val"),
      wisVal: el.querySelector(".wis-val"),
      lukVal: el.querySelector(".luk-val"),
      forVal: el.querySelector(".for-val"),
      pieVal: el.querySelector(".pie-val"),
      strBonus: el.querySelector(".str-bonus"),
      dexBonus: el.querySelector(".dex-bonus"),
      agiBonus: el.querySelector(".agi-bonus"),
      vitBonus: el.querySelector(".vit-bonus"),
      intBonus: el.querySelector(".int-bonus"),
      cncBonus: el.querySelector(".cnc-bonus"),
      wisBonus: el.querySelector(".wis-bonus"),
      lukBonus: el.querySelector(".luk-bonus"),
      forBonus: el.querySelector(".for-bonus"),
      pieBonus: el.querySelector(".pie-bonus"),
      totalDmgText: el.querySelector(".total-damage-text"),
      totalArmourText: el.querySelector(".total-armour-text"),
...
function updatePlayerVitals(ui, c, player) {
  ...
  if (ui.goldText && c.gold !== player.gold) {
    ui.goldText.textContent = player.gold.toFixed(2);
    c.gold = player.gold;
  }
}
...
function updatePlayerShopAndGear(ui, c, player, isOwnPlayer) {
  ...
  if (ui.totalArmourText && c._lastTotalArmour !== arm) {
    ui.totalArmourText.textContent = arm;
    c._lastTotalArmour = arm;
  }
}
...
function updatePlayerStats(ui, c, player) {
  ...
  stats.forEach((k, i) => {
    ...
    if (be) {
      const b = c._gearStats?.bonuses[k] ?? 0;
      const rounded = Math.round(b);
...
function updatePlayerDebuffs(ui, c, player) {
  ...
  if (c.debuffsHtml !== debuffsHtml) {
    if (ui.debuffs) ui.debuffs.innerHTML = debuffsHtml;
    c.debuffsHtml = debuffsHtml;
  }
}
...
function updatePlayerEquipped(el, c, player) {
  if (!el.dataset.collapsed && el._ui?.playerEquipped) {
    ...
    const snap = ["weapon", "chest", "helmet", "shoes"]
      ...
      .join("/");
    if (c._equipSnap !== snap) {
      el._ui.playerEquipped.innerHTML = renderEquippedHtml(player);
      c._equipSnap = snap;
    }
  }
}
window.updatePartyDisplay = updatePartyDisplay;
const WEAPON_EMOJI_MAP = { melee: "⚔️", ranged: "🏹", magic: "🔮" };
...
function buildEnemyContent(enemy) {
  return `
    <div class="stats enemy-stats">
      <strong>⚖️${enemy.level} ${getWeaponEmoji(enemy.equipment?.weapon)} ${enemy.name}</strong><br>
