...
function webrtcNoBatchLines() {
  return webrtcSent.filter((s) => s.type === "eventLog" && s.options.noBatch === true).map((s) => s.data.message);
}
...
assert(
  firstTcp.some((m) => m.includes("🏁")),
  "first clear: 🏁 must be sent over Socket.IO (TCP)"
);
assert(
  firstTcp.some((m) => m.includes("Returned to Town")),
  "first clear: return line must be sent over Socket.IO (TCP)"
);
assert(
  firstTcp.some((m) => /🎁|💰/.test(m)),
  "first clear: award line must be sent over Socket.IO (TCP)"
);
assert(
  firstUdp.some((m) => m.includes("🏁")),
  "first clear: 🏁 must be sent over WebRTC with noBatch"
);
assert(
  firstUdp.some((m) => m.includes("Returned to Town")),
  "first clear: return line must be sent over WebRTC with noBatch"
);
...
assert(
  secondTcp.some((m) => m.includes("🏁")),
  "second clear: 🏁 must still be sent over Socket.IO (was previously gated)"
);
assert(
  secondTcp.some((m) => m.includes("Returned to Town")),
  "second clear: return line must still be sent over Socket.IO"
);
assert(
  secondUdp.some((m) => m.includes("🏁")),
  "second clear: 🏁 must still be sent over WebRTC with noBatch"
);
