...
function parseStatus(text) {
  ...
  s.level = num(/Lv(\d+)/) ?? s.level;
  if (s.level === null) s.level = -1;
  s.gold = num(/Gold (-?[\d.]+)/) ?? s.gold;
  s.points = num(/Points (-?[\d.]+)/) ?? s.points;
  const hp = text.match(/HP (-?[\d.]+)\/(-?[\d.]+)/);
  if (hp) {
    s.hp = +hp[1];
    s.maxHp = +hp[2];
  }
  const mp = text.match(/MP (-?[\d.]+)\/(-?[\d.]+)/);
  if (mp) {
    s.mp = +mp[1];
    s.maxMp = +mp[2];
  }
  const ap = text.match(/AP (-?[\d.]+)\/(-?[\d.]+)/);
  if (ap) {
    s.ap = +ap[1];
    s.maxAp = +ap[2];
  }
  ...
  const cm = text.match(/combat (\S+)/);
  if (cm) s.combat = cm[1] === "true";
  const au = text.match(/autoEmbark (\S+)/);
  if (au) s.auto = au[1] === "true";
  const shop = text.match(/Shop: (\d+) items/);
  if (shop) s.shopCount = +shop[1];
  return s;
}
...
function descTier(desc) {
  const lv = desc.match(/Lv(\d+)/);
  const rar = desc.match(/(\d+)★/);
  const level = lv ? +lv[1] : 1;
  const rarity = rar ? +rar[1] : 1;
  const levelMul = 0.8 + level / 24,
    rarityMul = 0.7 + rarity / 13;
...
async function allocateAll() {
  while (true) {
    ...
    for (const [stat, amt] of plan) {
      if (st.points >= amt) {
        await run(`allocate ${stat} ${amt}`);
        spent = true;
        break;
      }
    }
    if (!spent) {
      if (st.points > 0) await run(`allocate vit ${st.points}`);
      break;
    }
  }
}
...
async function gearUp() {
  ...
  const equipBestFromInv = async (label) => {
    ...
    if (!invItems.length) {
      console.log(`[driver] ${label}: inventory empty.`);
      return;
    }
    const best = {};
  ...
  for (const it of fin) {
    const t = descTier(it.desc);
    if (!keepSlot[it.slot] || t > keepSlot[it.slot].t) keepSlot[it.slot] = it;
  }
  for (const it of fin) {
    if (keepSlot[it.slot]?.id !== it.id) await run(`sell ${it.id}`);
  }
...
async function safeToAdvance(idx, farmRuns) {
  ...
  console.log(
    `[driver] gate for next (idx ${idx + 1}): Lv${st.level}>=${needLvl}? ${st.level >= needLvl}; tier ♔${tier.toFixed(2)}>=♔${needTier}? ${tier >= needTier}; farmRuns=${farmRuns}/${cap} -> ${ok ? "SAFE" : "NOT SAFE, farming more"}`
  );
  return ok;
}
...
async function tryAdvance(next) {
  ...
  if (st.dungeon === next) {
    console.log(`[driver] advanced to ${next}.`);
    return true;
  }
...
async function main() {
  ...
  if (!joined) {
    console.log("[driver] never joined; abort.");
    child.kill();
    return;
  }
  ...
  while (idx < DUNGEON_ORDER.length) {
    ...
    if (await safeToAdvance(idx, farmRuns)) {
      if (await tryAdvance(DUNGEON_ORDER[idx + 1])) {
        idx++;
        farmRuns = 0;
      } else {
        console.log(`[driver] change failed; farming ${target} again.`);
      }
    } else {
