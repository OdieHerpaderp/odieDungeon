...
const skillFiles = fs.readdirSync(path.join(__dirname, "..", "public", "abilities")).filter((f) => /^skill_.*\.json$/.test(f));
...
assert.ok(
  armorAbilities.every((a) => a.defenseUpAmount && a.defenseUpDuration),
  "every armor ability must be a defenseUp self-buff"
);
