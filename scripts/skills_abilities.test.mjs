...
const ability = {
  id: "arcane_bolt",
  skillId: "skill_magic",
  unlockSkillLevelMin: 1,
  mpCostBase: 4,
  cooldownMsBase: 2000,
  allowedWeaponClasses: ["magic"],
};
const player = {
  mp: 10,
  skillsState: skillMagicState,
  abilityCooldowns: {},
  equipment: { weapon: { weaponClass: "magic" } },
  abilitySlots: ["arcane_bolt"],
};
...
const firstAid = {
  ...
  unlockSkillLevelMin: 0,
  mpCostBase: 6,
  cooldownMsBase: 2500,
  allowedWeaponClasses: ["magic", "ranged", "melee"],
  requiresWeaponEquipped: false,
  isHeal: true,
};
...
const shortBladeAbility = {
  id: "shortblade_backstab",
  skillId: "skill_melee_shortBlade",
  unlockSkillLevelMin: 1,
  mpCostBase: 6,
  cooldownMsBase: 2000,
  allowedWeaponClasses: ["melee"],
  requiredWeaponSubTypes: ["shortBlade"],
  requiresWeaponEquipped: true,
};
const daggerPlayer = {
  mp: 10,
  skillsState: shortBladeSkillState,
  abilityCooldowns: {},
  equipment: { weapon: { weaponClass: "melee", subType: "shortBlade" } },
  abilitySlots: ["shortblade_backstab"],
};
...
const swordPlayer = {
  ...
  skillsState: shortBladeSkillState,
  abilityCooldowns: {},
  equipment: { weapon: { weaponClass: "melee", subType: "longBlade" } },
  abilitySlots: ["shortblade_backstab"],
};
...
assert.ok(
  selectAbilityToCast(healerPlayer, [healAbility], Date.now(), [fullHealthAlly, woundedAlly]),
  "heal should be selectable when an ally is below 75% HP"
);
assert.strictEqual(
  selectAbilityToCast(healerPlayer, [healAbility], Date.now()),
  healAbility,
  "heal should be selectable when liveTargets is omitted (backward compat)"
);
...
const survivalBandagePlayer = {
  mp: 10,
  skillsState: awardSkillXp(getDefaultSkillsState(), "skill_survival", 200),
  abilityCooldowns: {},
  abilitySlots: ["survival_bandage"],
  equipment: {},
};
...
assert.strictEqual(
  selectAbilityToCast(survivalBandagePlayer, [bandage], Date.now(), [survivalFullAlly]),
  null,
  "survival heal should be blocked when no ally needs healing"
);
...
const callSpirit = {
  id: "shaman_call_spirit",
  skillId: "skill_shamanism",
  unlockSkillLevelMin: 1,
  mpCostBase: 6,
  cooldownMsBase: 2800,
  allowedWeaponClasses: ["magic"],
...
const shamanPlayer = {
  mp: 15,
  skillsState: awardSkillXp(getDefaultSkillsState(), "skill_shamanism", 200),
  abilityCooldowns: {},
  equipment: { weapon: { weaponClass: "magic" } },
  abilitySlots: ["shaman_call_spirit"],
};
...
const blessedTouch = {
  id: "miracle_blessed_touch",
  skillId: "skill_miracles",
  unlockSkillLevelMin: 2,
  mpCostBase: 8,
  cooldownMsBase: 3200,
  allowedWeaponClasses: ["magic"],
  requiresWeaponEquipped: true,
...
const miraclePlayer = {
  mp: 25,
  skillsState: awardSkillXp(getDefaultSkillsState(), "skill_miracles", 400),
  abilityCooldowns: {},
  equipment: { weapon: { weaponClass: "magic" } },
  abilitySlots: ["miracle_blessed_touch"],
};
