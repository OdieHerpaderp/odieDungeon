...
export class WebRTCServer extends EventEmitter {
  ...
  setupDefaultEventHandlers() {
    ...
    handlers.iceCandidate = (s, c) => {
      if (this.io) this.io.to(s).emit("webrtc-signal", { candidate: c });
    };
  ...
  getBatchStats() {
    const stats = {
      ...
      clientsWithPreferences: this.clientBatchPreferences.size,
      timerStatus: {
  ...
  flushBatchQueue() {
    ...
    this.batchQueues.forEach((accumulators, socketId) => {
      ...
      try {
        ...
        console.error(`[WebRTC Batch] Flush error for ${socketId}:`, error.message, `(${accumulator.count} pending messages cleared)`);
  ...
  stopBatchTimers() {
    Object.entries(this.batchTimers).forEach(([p, t]) => {
      if (t) {
        clearInterval(t);
  ...
  closePeerConnection(socketId) {
    ...
    try {
      ...
      console.error(`[WebRTC] Error closing data channel for ${socketId}:`, e);
    }
    ...
    try {
      ...
      console.error(`[WebRTC] Error closing peer connection for ${socketId}:`, e);
    }
  ...
  async handleIceCandidate(socketId, candidate) {
    ...
    if (peerData?.pc && candidate?.candidate) {
      try {
        ...
        console.error(`Failed to add ICE candidate for ${socketId}:`, error);
      }
    }
  }
  ...
  getPeerStats() {
    ...
    this.peers.forEach((peerData, socketId) => {
      ...
      stats.peers.push({
        ...
        connectionState: peerData.pc?.connectionState || "unknown",
        dataChannelState: peerData.dataChannel?.readyState || "unknown",
        createdAt: peerData.createdAt,
        messageCount: peerData.messages.length,
        batchQueued: queuedByPriority,
      });
    });
